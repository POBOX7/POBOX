...
use Session;
class KurtiesController extends Controller {

  public function KurtiesList() 
  { 
    $bannerSlider = Banners::where('page_id',4)->first(); 
    $newArrivalProductData = Product::where('category_id',1)->where('status',1)->where('is_deleted',0)->paginate(9);
    //dd($newArrivalProductData);
    $newArrivalProductDatas = Product::where('category_id',1)->where('status',1)->where('is_deleted',0)->paginate(9);
    
    foreach ($newArrivalProductDatas as $keyNewArrivalProductReview => $valueNewArrivalProductReview) {
      $productSku = Product::where('id',$valueNewArrivalProductReview->id)->where('category_id','1')->where('status','1')->where('is_deleted','0')->pluck('sku')->toArray();
   ...
   die();*/
    $newArrivalHighlightProductData = Product::where('category_id','1')->where('highlight','1')->where('status','1')->where('is_deleted','0')->get();
   ...
   //Left side bar 
    ...
    foreach ($brandsData as $keyBrandsData => $valueBrandsData) {
      ...
      $brandsData[$keyBrandsData]['product_count'] = $product = Product::where('brand_id',$valueBrandsData['id'])->where('category_id','1')->where('status','1')->where('is_deleted','0')->count('brand_id');
    }

    //FEATURED PRODUCTS
     $featuredProduct = Product::where('category_id','1')->where('is_featured','1')->where('status','1')->where('is_deleted','0')->take(3)->get();
    ...
    return view('new_resources.kurties',compact('bannerSlider','newArrivalProductDatas','newArrivalHighlightProductData','categoryData','brandsData','featuredProduct','sizeData','colorsData','sizeInformation','left_sidebar_featuredProduct')); 
  }  
