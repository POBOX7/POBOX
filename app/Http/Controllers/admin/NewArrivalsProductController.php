...
class NewArrivalsProductController extends Controller 
{
    /**
     ...
     */
    public function newArrivalsProductIndex()
    {
        $productList = DB::table('products')
                            ...
                            ->select(\DB::raw('(SELECT GROUP_CONCAT(qty) as `qty` FROM product_size WHERE product_size.product_id = products.id) AS qty,(SELECT GROUP_CONCAT(name) as `size_name` FROM product_size INNER JOIN sizes ON sizes.id = product_size.size_id WHERE product_size.product_id = products.id) AS size_name,
                                products.id,products.image,products.barcode,products.status,products.name,products.sku,products.price,colors.hex_code'))
                            ->where('products.is_deleted','0')
                            ->where('products.is_new_arrival','1')
                            ->orderByDesc('id')
                            ->get();
        return view('admin.new_arrivals_products.product_index')->with('productList', $productList); 
    }

    public function addProductNewArrivals( Request $request ) {
    	...
    	return view('admin.new_arrivals_products.product_add')->with('categoryList', $categoryList)->with('brandList', $brandList)->with('colorList', $colorList)->with('sizeList', $sizeList);
    }

    public function bulkUploadNewArrivals( Request $request ) {
        return view('admin.new_arrivals_products.product_csv_import');
    }

    public function storeBulkUploadNewArrivals( Request $request ) {    
        ...
        return redirect()->route('newArrivalsProductIndex')->with('success', 'Product created successfully.');
     ...
     * @return \Illuminate\Contracts\Support\Renderable
     */
    public function storeProductNewArrivals( Request $request ) {    
        $validatedData = $request->validate([
       ...
       $image_resize->resize(277, 400);
        ...
        return redirect()->route('newArrivalsProductIndex')->with('success', 'Product added successfully.');
        
    }

    public function editProductNewArrivals(Request $request ,$id)
    {
        $id = base64_decode($id);
        ...
        return view('admin.new_arrivals_products.product_edit',compact('productDetail','id','categoryList','brandList','colorList','sizeList'));
    }

    public function viewProductNewArrivals(Request $request ,$id)
    {
        $id = base64_decode($id);
        ...
        return view('admin.new_arrivals_products.product_view',compact('productDetail','id','sizeList'));
    }

    public function printProductNewArrivals($id)
    {
...
//dd($barcode);
    ...
    public function updateProductNewArrivals(Request $request ,$id)
    {
        $request->validate([
       ...
       if (isset($request->image)) {
        ...
        return redirect()->route('newArrivalsProductIndex')->with('success', 'Product details updated successfully.');
    }

    public function productDestroyNewArrivals($id)
    {
        $update_data = array(
                                'is_deleted' => 1,
                                'updated_at'    => date('Y-m-d H:i:s')
        ...
        ->update($update_data);
        return redirect()->route('newArrivalsProductIndex')->with('success', 'Product removed successfully.');
    }

    public function productStatusNewArrivals()
    {
        extract($_POST);
    ...
    public function productImageDestroyNewArrivals($id)
    {
        ProductImage::where('id',$id)->delete();
        echo '1';
    }

    public function allProductNewArrivals()
    {
    ...
    public function allSizeNewArrivals()
    {
    ...
    public function exportNewArrivals(Request $request) 
    {
