   ...
   public function addToCart(Request $request){
	...
	$check_list = Cart::where('user_id',$request->user_id)
      ...
      if($check_list){
		...
		$size = ProductSize::where('product_id',$request->product_id)->where('size_id',$request->size)->first();
		if($size->qty != 0)
		{
			$size->qty = $size->qty - $request->qty;
			$size->save();
    ...
    public function removeProduct($id){
	 ...
	 if(Auth::check())
	 {
      ...
      $cart_data  = Cart::find($id);
	   ...
	   $size = ProductSize::where('product_id',$cart_data->product_id)->where('size_id',$cart_data->size)->first();
	   $size->qty = $size->qty + $cart_data['qty'];
	   $size->save();
        ...
        $cart_data->delete(); 
		 ...
		 foreach (array_filter(Session::get('shoppingCart')) as $key => $value) {
			...
			if ($value['cart_id'] == $id) {
				$size = ProductSize::where('product_id',$value['user_product_id'])->where('size_id',$value['user_size'])->first();
			    $size->qty = $size->qty + $value['user_qty'];
				$size->save();
			    session()->forget("shoppingCart.$key");
       ...
       if(Auth::check())
	   {
            ...
            $cart_data  = Cart::where('user_id',$user_id)->get();
			 ...
			 foreach ($cart_data as $key => $value) {
				...
				$size = ProductSize::where('product_id',$value['product_id'])->where('size_id',$value['size'])->first();
			  $size->qty = $size->qty + $value['qty'];
				$size->save();
		 	}
